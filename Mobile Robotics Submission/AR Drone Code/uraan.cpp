...
int main () {
   ...
   //~ Change FPS
   //~ sendAT("AT*CONFIG=1,\"video:codec_fps\",\"10\"\r");
   ...
   while(1){
        ...
        if(image.empty() || (camCount++)%4!=0) continue; // Capture every fourth frame
        //~ printf("camCount = %u\n", camCount);
   ...
   if(filterSquares(squares, gSquares)){ // Marker Found
         ...
         float area = fabs(contourArea(Mat(gSquares[1])));
         float dis = sqrt(area);
         cout<<"Dist: "<<dis<<endl;
         ...
         if(abs(tError.FB) < 10){
            ...
            if(tError.FB > 0) // Back
               cmd_vel.FB = kp * tError.FB * 15;
            else // Forward
               cmd_vel.FB = kp * tError.FB * 25;
         }
         cout<<"Error = "<<tError.FB<<endl;
